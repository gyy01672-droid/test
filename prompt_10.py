...
def build_prompt(sentence, k=3):
    ...
    prompt = f"""请将句子分类为以下情感类别之一：{labels_str}

"""
    for _, row in sample.iterrows():
        prompt += f"句子：{row['sentence']}\n情感类别：{row['Label']}\n\n"
...
for k in [1, 3, 5]:
    print(f"\n k = {k} ")
    result = evaluate(df_test, k=k, max_samples=50)
    print(f"结果: Acc = {result['accuracy']:.4f}, F1 = {result['f1_macro']:.4f}")